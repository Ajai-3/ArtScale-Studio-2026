...
const Workspace = () => {
  ...
  const renderHorizontalRuler = () => {
    ...
    const maxCm = Math.ceil(width / spacing) + 2;
    ...
    for (let mm = 0; mm <= endMm; mm += stepMm) {
      ...
      ticks.push(
        <line key={`ht-${mm}`} x1={x} y1={RULER_SIZE - tickH} x2={x} y2={RULER_SIZE}
          stroke="#ABF600" strokeWidth={isMajor ? 1.5 : 1} opacity={isMajor ? 0.9 : isMid ? 0.6 : 0.3} />,
      );

      if (isMajor && cm >= 0) {
        ticks.push(
          <text key={`hl-${mm}`} x={x + 2} y={12} fontSize="9"
            fill="#ABF600" fontFamily="monospace" fontWeight="bold" opacity="0.9">{cm}</text>,
        );
      } else if (isMid && cm > 0 && spacing >= 20) {
        ticks.push(
          <text key={`hl-mid-${mm}`} x={x + 2} y={15} fontSize="8"
            fill="#ABF600" fontFamily="monospace" opacity="0.6">{cm}</text>,
        );
      }
    }
    return ticks;
  ...
  const renderVerticalRuler = () => {
    ...
    const spacing = pixelsPerCm * zoomLevel;
    const maxCm = Math.ceil(height / spacing) + 2;
    ...
    for (let mm = 0; mm <= endMm; mm += stepMm) {
      ...
      ticks.push(
        <line key={`vt-${mm}`} x1={RULER_SIZE - tickW} y1={y} x2={RULER_SIZE} y2={y}
          stroke="#ABF600" strokeWidth={isMajor ? 1.5 : 1} opacity={isMajor ? 0.9 : isMid ? 0.6 : 0.3} />,
      );

      if (isMajor && cm >= 0) {
        ticks.push(
          <text key={`vl-${mm}`} x={2} y={y + 10} fontSize="9"
            fill="#ABF600" fontFamily="monospace" fontWeight="bold" opacity="0.9">{cm}</text>,
        );
      } else if (isMid && cm > 0 && spacing >= 20) {
        ticks.push(
          <text key={`vl-mid-${mm}`} x={2} y={y + 8} fontSize="8"
            fill="#ABF600" fontFamily="monospace" opacity="0.6">{cm}</text>,
        );
      }
    }
